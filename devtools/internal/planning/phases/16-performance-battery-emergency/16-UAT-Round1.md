...
- truth: "Typing quickly in any application (like Notes, Xcode, or a browser) feels completely native and fluid. There are no dropped keystrokes, stuttering, or UI lockups caused by the background event tap."
  ...
  severity: major
  test: 1
  root_cause: "AXUIElement fetches are flooding the background queue on every keystroke. Spacebar events are being delayed by queue contention. QOS is too high, competing with live typing thread."
  artifacts:
    - path: "TypeFlow/Services/AccessibilityMonitor.swift"
      issue: "AXUIElement fetching not throttled, Spacebar not bypassing background queue, QOS is userInteractive"
  missing:
    - "Throttle context fetching to only space/punctuation or 150ms debounce"
    - "Bypass background queue for keyCode 49 (spacebar) and return"
    - "Downgrade processingQueue QOS to .utility"
