...
def parse_logs(log_file):
    ...
    previousContextLens = []
    previousLinesIncludedCounts = {}
    activeLineOnlyCount = 0
    tooManyLinesWarnings = 0
    ...
    with open(log_file, 'r') as f:
        for line in f:
            if '[PromptContextWindow]' in line:
                if 'warning=tooManyLinesForInlineAutocomplete' in line:
                    tooManyLinesWarnings += 1
                else:
                    sc_match = re.search(r'promptSuffixLineCount=(\d+)', line)
                    pcl_match = re.search(r'previousContextLen=(\d+)', line)
                    pli_match = re.search(r'previousLinesIncluded=(\d+)', line)
                    reason_match = re.search(r'previousContextOmittedReason=([\w]+)', line)
                    ...
                    if pli_match:
                        pli = int(pli_match.group(1))
                        previousLinesIncludedCounts[pli] = previousLinesIncludedCounts.get(pli, 0) + 1
                    if reason_match and reason_match.group(1) == 'activeLineOnly':
                        activeLineOnlyCount += 1
    ...
    print(f"PromptContextWindow counts:")
    ...
    print(f"  previousLinesIncluded counts: {previousLinesIncludedCounts}")
    print(f"  activeLineOnly count: {activeLineOnlyCount}")
    print(f"  tooManyLinesForInlineAutocomplete warnings: {tooManyLinesWarnings}")
    ...
    print(f"\nVisibleSuggestionAudit visibleApplied count: {visible_audit_count}")
